...
test("106537 - Restricted movement doesn't work @coreTrayView", async ({ }, testInfo) => {
  //set timeout 
  ...
  //user clicks on core tray view tab 
    await test.step(`User clicks on core tray view tab `, async () => {
      ...
      await coreTrayView.pause(10000);
    });

  //user clicks on menu icon 
    await test.step(`User clicks on menu icon`, async () => {
        await coreTrayView.clickToElement(coreTrayView.menuIcon);
        await coreTrayView.pause(10000);
    });

 //expect the restricted movement option is removed
    await test.step(`User clicks on menu icon`, async () => {
       await coreTrayView.waitForElement(
        coreTrayView.restrictedMovement,
        "hidden",
        300000
       )
    });
