...
test("COR-367 - Full Screen Drill Hole Viewer @drillhole @viewPanel", async ({ }, testInfo) => {
  //set timeout 
  ...
  //expect "Full Screen" element is displayed 
  await test.step(`Expect "Full Screen" element is displayed `, async () => {
    await drillHoleView.waitForElement(
      drillHoleView.uncheckedFullScreen,
      "visible",
      300000
    )
  });

  //user clicks on "Full Screen" checkbox 
  await test.step(`User clicks on "Full Screen" checkbox  `, async () => {
    await drillHoleView.clickToElement(drillHoleView.uncheckedFullScreen);
    await drillHoleView.pause(5000);
  });

  //expect opened "Full Screen"
  await test.step(`Expect opened "Full Screen"`, async () => {
    await drillHoleView.clickToElement(drillHoleView.menuBtn);
    await drillHoleView.pause(2000)
    await drillHoleView.waitForElement(
      drillHoleView.checkedFullScreen,
      "visible",
      300000
    )
  });
