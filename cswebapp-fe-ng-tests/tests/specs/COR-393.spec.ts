...
test("71134 - Project / Drillhole panel issues @dashboard", async ({ }, testInfo) => {

  //set timeout 
  test.setTimeout(300000);

  //user search "william2-dh1" drill hole 
  await test.step(`User search "william2-dh1" drill hole`, async () => {
    await dashBoardPage.pause(5000)
    await dashBoardPage.waitForElement(
      dashBoardPage.drillHoleSearchResult("william2-dh1"),
      "visible",
      300000
    )
  });

  //expect when the drill hole is displayed then the loading message will hidden 
  await test.step(`User search "william2-dh1" drill hole`, async () => {
    await dashBoardPage.pause(3000);
    await dashBoardPage.waitForElement(
      dashBoardPage.loadingMessage,
      "hidden",
      30000
    )
