...
test("32445 - Panel is open when a different category is selected @drillhole", async ({ }, testInfo) => {
  //set timeout 
  ...
  //user selects category and verify panel is correctly displayed #1
  await test.step(`User selects category and verify panel is correctly displayed #1`, async () => {
  ...
  //user select category and verify panel is correctly displayed #2
  await test.step(`User select category and verify panel is correctly displayed #2`, async () => {
    await drillHoleView.clickToElement(drillHoleView.categoryDropdown);
    await drillHoleView.pause(2000);
    await drillHoleView.clickToElement(drillHoleView.dynamicCategoryType("Mineral Composition"))
    await drillHoleView.pause(2000);
    await drillHoleView.countElement(drillHoleView.dynamicValuePanel(), 1);
  });
