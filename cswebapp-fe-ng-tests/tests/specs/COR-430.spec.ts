...
test("85418 - Activating Core Tray Viewer Image Enhancement resets the view to stacked tray view @coreTrayView", async ({ }, testInfo) => {
  //set timeout 
  ...
  //user clicks on "Menu item" dropdown 
  await test.step(`User clicks on "Menu item" dropdown `, async () => {
    await drillHoleView.clickToElement(drillHoleView.midleMenuDropdown);
  await drillHoleView.pause(3000);
  });

  //user clicks on "Image Enhancement" checkbox
   await test.step(`User clicks on "Image Enhancement" checkbox`, async () => {
