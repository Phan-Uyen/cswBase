...
test("85414 - Core Tray Viewer Switch to 3D mode results in the user @coreTrayView", async ({}, testInfo) => {
//set timeout 
  ...
  //user selects "Menu items" dropdown 
  await test.step(`User selects "Menu items" dropdown `, async () => {
    await coreTrayView.clickToElement(coreTrayView.dynamicItemproducType('0'));
    await coreTrayView.pause(3000);
  });

  //expect "Switch to 3D" option  is not displayed
  await test.step(`Expect "Switch to 3D" option  is not displayed`, async () => {
    await coreTrayView.waitForElement(
      coreTrayView.switchTo3DMode,
      "hidden",
      300000
    )
  });
