...
test("106540 - Dashboard filter text state not preserved but filter is still preserved when transitioning from drillhole viewer to dashboard @viewDrillHole", async ({ }, testInfo) => {
  //set timeout 
  ...
  //user back to dashboard page 
  await test.step(`User back to dashboard page `, async () => {
    dashBoardPage.clickToElement(dashBoardPage.dashboardTitle);
    await dashBoardPage.pause(5000);
  });

  // get value and expect value in filter text is displayed 
  const getValueFilterText = page.locator("(//input[@placeholder ='Input search text'])[2]");
  console.log(getValueFilterText)
  await expect(getValueFilterText).toHaveValue('WINU0725');
  ...
  //clear data in search drill hole field 
  await test.step(`User clear data in search drill hole field `, async () => {
    await dashBoardPage.clearValue(dashBoardPage.searchDrillHoleInput);
  });

  //user scroll down and select "WIN0725" 
  await test.step(`User scroll down and select "WIN0725"  `, async () => {
    await dashBoardPage.scrollToElement(dashBoardPage.drillHoleSearchResult("WINU0725"));
    await dashBoardPage.clickToElement(
      dashBoardPage.drillHoleSearchResult("WINU0725")
    );
  ...
  //expect the text show 1 To 1 of Entries is not displayed
  await test.step(`Expect the text show 1 To 1 of Entries is not displayed`, async () => {
    await dashBoardPage.waitForElement(
      dashBoardPage.show1To1of1Entries,
      "hidden",
      300000
    )
