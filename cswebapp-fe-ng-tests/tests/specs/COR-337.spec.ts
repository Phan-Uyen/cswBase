...
test("39208 - The images are displayed differently each time users open drill hole view tab @drillhole ", async ({}, testInfo) => {
    //set timeout 
  ...
  await test.step(`User selects unzoom option`, async () => {
    ...
    await test.step(`User goes to the dashboard`, async () => {
      await dashBoardPage.clickToElement(dashBoardPage.dashboardTitle); 
      await drillHoleView.pause(8000);
  
    });

    //user goes to drill hole view tab again
    await test.step(`User goes to drill hole view tab again`, async () => {
      await drillHoleView.clickToElement(drillHoleView.drillHoleViewTab);
      await drillHoleView.reloadPage();
      await drillHoleView.pause(5000);
    });

    //user snapshort and compare image 
    await test.step(`User snapshort and compare image `, async () => {
