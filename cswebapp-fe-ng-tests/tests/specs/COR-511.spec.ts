...
test("113005 - When add dep scale panel ,the display not correct", async ({ }, testInfo) => {
    //set timeout 
    ...
    await test.step(`User selects category Depth Scale`, async () => {
        await drillHoleView.clickToElement(drillHoleView.categoryTypeDropDown);
        await drillHoleView.clickToElement(
            drillHoleView.depthScaleCategory
        );
        await drillHoleView.pause(3000);
    });

    const currentCategory = page.locator("//input[@id='viewer2-filter-category']");
    console.log(currentCategory);
    await expect(currentCategory).toHaveValue("Depth Scale");
    await drillHoleView.pause(3000);
});
