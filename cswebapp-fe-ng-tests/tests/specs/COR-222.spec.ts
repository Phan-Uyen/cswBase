...
test("39335 - Region key identification review @coreTrayview", async ({ }, testInfo) => {
  //set timeout 
  ...
  //expect the core tray panel is correctly displayed
  await test.step(`Expect the core tray panel is correctly displayed`, async () => {
    await coreTrayView.countElement(coreTrayView.trayViewPanel, 1);
    await coreTrayView.pause(2000);
  });

  //expect the error message is not displayed
  await test.step(`Expect the error message is not displayed`, async () => {
